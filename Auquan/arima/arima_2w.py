...
df_ADANI = df_ADANI[df_ADANI.index%120 == 0]
#predictions = pd.DataFrame(index = df_ADANI.index, columns = stocks)


# step = 12
df_outer = pd.read_table('/home/saumya/Auquan/stocklist_2w', names=('A'))
stocks = df_outer['A']
...
for stockname in df_outer['A']:    
                    ...
                    rng = pd.date_range('2017-01-02 09:16:00', periods=500, freq='120min')
                    df = df.iloc[0:60000]
                    df = df[df.index%120 == 0]
    ...
    #                                     print("no error")
                        ...
                        if best_mdl is None:                                 
                                    ...
                                    #print(k)
                                    predict = best_mdl.forecast(steps = 80, alpha = 0.01)[0]
